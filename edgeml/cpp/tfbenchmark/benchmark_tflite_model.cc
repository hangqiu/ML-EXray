...
#include "edgeml/cpp/tfbenchmark/benchmark_utils.h"
...
BenchmarkParams BenchmarkTfLiteModel::DefaultParams(string summary_log_file) {
  ...
  default_params.AddParam("profiling_output_csv_file",
                          BenchmarkParam::Create<std::string>(summary_log_file));
...
TfLiteStatus BenchmarkTfLiteModel::Init() {
  ...
  return InitBenchmarkModel();
}

TfLiteStatus BenchmarkTfLiteModel::InitBenchmarkModel() {
  ...
  if (profiling_listener_) {
    TFLITE_LOG(INFO) << "Operation profiler created";
//    __android_log_print(ANDROID_LOG_ERROR, "tflite", "Operation profiler created");
    AddListener(profiling_listener_.get());
  }else{
    TFLITE_LOG(INFO) << "Operation profiler not created";
//    __android_log_print(ANDROID_LOG_ERROR, "tflite", "Operation profiler not created");
  }
...
TfLiteStatus BenchmarkTfLiteModel::InitBenchmarkModel(std::unique_ptr<tflite::Interpreter>* interpreter_ptr) {
  // Install profilers if necessary right after interpreter is created so that
  // any memory allocations inside the TFLite runtime could be recorded if the
  // installed profiler profile memory usage information.
  profiling_listener_ = MayCreateProfilingListener(interpreter_ptr);
  if (profiling_listener_) {
    TFLITE_LOG(INFO) << "Operation profiler created";
//    __android_log_print(ANDROID_LOG_ERROR, "tflite", "Operation profiler created");
    AddListener(profiling_listener_.get());
...
//    __android_log_print(ANDROID_LOG_ERROR, "tflite", "Operation profiler not created");
  ...
  interpreter_state_printer_ = std::unique_ptr<BenchmarkListener>(
          new InterpreterStatePrinter((*interpreter_ptr).get()));
  AddListener(interpreter_state_printer_.get());

  (*interpreter_ptr)->SetAllowFp16PrecisionForFp32(params_.Get<bool>("allow_fp16"));
  ...
  for (const auto& delegate_provider :
    ...
    // user-specified benchmark params tells not to.
    if (delegate == nullptr) continue;
    if ((*interpreter_ptr)->ModifyGraphWithDelegate(delegate.get()) != kTfLiteOk) {
      TFLITE_LOG(ERROR) << "Failed to apply " << delegate_provider->GetName()
                        << " delegate.";
      return kTfLiteError;
    } else {
      ...
      // delegate is being applied to the model graph.
      int num_delegated_kernels = 0;
      for (int i = 0; i < (*interpreter_ptr)->execution_plan().size(); ++i) {
        int node_id = (*interpreter_ptr)->execution_plan()[i];
        if (checked_node_ids.find(node_id) != checked_node_ids.end()) {
          continue;
        }
        const TfLiteNode& node =
                (*interpreter_ptr)->node_and_registration(node_id)->first;
      ...
      bool fully_delegated = (num_delegated_kernels == 1 &&
              (*interpreter_ptr)->execution_plan().size() == 1);
  ...
  auto interpreter_inputs = (*interpreter_ptr)->inputs();
  ...
  for (int j = 0; j < inputs_.size(); ++j) {
    const InputLayerInfo& input = inputs_[j];
    int i = interpreter_inputs[j];
    TfLiteTensor* t = (*interpreter_ptr)->tensor(i);
    if (input.name != t->name) {
      TFLITE_LOG(WARN) << "Tensor # " << i << " is named " << t->name
                       << " but flags call it " << input.name;
    }
  ...
  for (int j = 0; j < inputs_.size(); ++j) {
    ...
    TfLiteTensor* t = (*interpreter_ptr)->tensor(i);
    if (t->type != kTfLiteString) {
      (*interpreter_ptr)->ResizeInputTensor(i, input.shape);
    }
  }
...
std::unique_ptr<BenchmarkListener>
BenchmarkTfLiteModel::MayCreateProfilingListener(std::unique_ptr<tflite::Interpreter>* interpreter_ptr) const {
  if (!params_.Get<bool>("enable_op_profiling")) return nullptr;

  return std::unique_ptr<BenchmarkListener>(new ProfilingListener(
          (*interpreter_ptr).get(), params_.Get<int32_t>("max_profiling_buffer_entries"),
          params_.Get<std::string>("profiling_output_csv_file"),
          CreateProfileSummaryFormatter(
                  !params_.Get<std::string>("profiling_output_csv_file").empty())));
}
