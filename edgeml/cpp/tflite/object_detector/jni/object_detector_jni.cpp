...
#include "edgeml/cpp/tflite/object_detector/ObjectDetector.h"
...
Java_com_edgeml_edgemlinsight_NativeDetector_initDetector(
        JNIEnv *env,
        jobject p_this,
    ...
    jlong obj = (jlong) new ObjectDetector(
            buffer,
            nativeString,
            size,
            playback,
...
Java_com_edgeml_edgemlinsight_NativeDetector_detect(JNIEnv *env, jobject p_this, jlong detectorAddr,
                                                    jbyteArray src, int width, int height,
                                                    int rotation) {
    // Frame bytes to Mat
...
extern "C" JNIEXPORT jfloatArray JNICALL
Java_com_edgeml_edgemlinsight_NativeDetector_playbackdetect(JNIEnv *env, jobject p_this,
                                                            jlong detectorAddr,
                                                            jobject obj_bitmap) {
    // Frame bytes to Mat
    cv::Mat matrix;
    ...
    if (bitmapInfo.format == ANDROID_BITMAP_FORMAT_RGBA_8888) {
        ...
        cv::cvtColor(tmp, matrix, cv::COLOR_RGBA2BGRA);
    } else {
    ...
    // Detect
    ObjectDetector *detector = (ObjectDetector *) detectorAddr;
    DetectResult *res = detector->detect(matrix);
